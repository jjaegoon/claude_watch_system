...
## tsx + Node v24 ESM strict named export 검출 실패 (#9)
...
**원인 (가설)**:
- tsx의 ESM loader가 packages/db의 inline export 패턴을 named export로 인식 못함
- Node v24 ESM strict가 explicit `exports` field 없는 workspace 패키지를 fallback 처리 안 함
- packages/db package.json의 `type` 또는 `exports` field 부재 가능성

**임시 우회**: apps/api/src/lib/db.ts 자체 better-sqlite3 connection (gotcha #8 정합 — 4 PRAGMA set). 같은 dev.db (WAL 멀티 connection 안전).

**정식 해결**: T-40 ADR 후보 — packages/db ESM export 패턴 정식 결정. Step 4 진입 직전 의무 해결.

**연관**: gotcha #8 (multi-connection PRAGMA 재설정) — 임시 우회 시 자동 발현

**발견**: M1 Step 2, 2026-05-02 (Step 2 episode §새 발견)

-- Phase-O hotfix: usage_events event_type CHECK에 'asset_download' 추가
-- asset_install → asset_download 로 L2 이벤트 명칭 정합 (R-G9 §2.24 U-C1)
-- asset_install은 기존 데이터 호환 목적으로 유지
-- SQLite CHECK 수정 불가 → 테이블 재생성 패턴
...
CREATE TABLE IF NOT EXISTS usage_events_new (
  ...
  event_type      TEXT    NOT NULL
                  CHECK (event_type IN (
                    ...
                    'skill_trigger', 'asset_view', 'asset_install', 'asset_download',
                    'review_action'
                  )),
...
INSERT INTO usage_events_new
  SELECT id, user_id, session_id, event_type, asset_id, tool_name, file_path, ts, metadata,
         review_metadata
  FROM usage_events;
